...
def min_max_normalize(data):
    ...
    """
    # Separate the first two columns as labels
    labels = data.iloc[:, :2]

    # Select the rest of the columns for scaling
    data_to_scale = data.iloc[:, 2:]
...
# Example usages
data = pd.read_csv('TestDFFT/TestDfft(Quartz-Corona).csv')
normalized_data = min_max_normalize(data)
normalized_data.to_csv('TestDFFT/TestDfft(Quartz-Corona).csv')
