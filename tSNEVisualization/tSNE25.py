...
# Ensure each label is added to the legend
for label in unique_labels:
    df_label = df[df['machine'] == label]
    if len(df_label) > 0:
        first_instance = df_label.iloc[0]
        fig.add_trace(go.Scatter(
            x=[first_instance['tsne-2d-one']],
            y=[first_instance['tsne-2d-two']],
            mode='markers',
            marker=dict(
                symbol=marker_map[first_instance['sample_type']],
                size=10,
